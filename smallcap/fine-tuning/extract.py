...
annotations = json.load(open('datastore/index.json'))['images']
...
def load_data():
    ...
    for item in annotations:
        ...
        if item['split'] == 'train':
            data['train'].append({'file_name': file_name, 'cocoid': item['file_path']})
        elif item['split'] == 'val':
            data['val'].append({'file_name': file_name, 'cocoid': item['file_path']})
    return data
